...
public class registrarServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Consultas cov = new Consultas();
        if (cov.datosUsuario(Integer.toString(documento))) {
                            objetosesion.setAttribute("informacionE", "Usuario ya existe");
            ...
            if (co.registrar(documento, primNombre, segNombre, primApellido, segApellido, email, rol, password)) {
                ...
                String url = "http://" + address.getHostAddress() + ":8080/SOSDocumental/autenticacion.jsp";
                ...
                if (enviado) {
                    objetosesion.setAttribute("informacionV", "Registro Exitoso");
                    response.sendRedirect("registro.jsp");
                } else {
                    objetosesion.setAttribute("informacionE", "Se realizó el registro, pero no se envió el correo");
                    response.sendRedirect("registro.jsp");
                }
            } else {
                objetosesion.setAttribute("informacionE", "No se logró realizar el registro ");
                response.sendRedirect("registro.jsp");
            }
        }
    }
